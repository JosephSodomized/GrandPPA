...
class ChangeUsername extends Component {
  ...
  removingContactHandler = (email, name, number) => {
    Alert.alert(
      "Are you sure?",
      "Your contact will be removed and you no longer will be able to call it in case of emergency",
      [
        { text: "Cancel", onPress: () => {}, style: "cancel" },
        {
          text: "Delete",
          onPress: () => this.removeContact(email, name, number)
        }
      ]
    );
  };
  ...
  render() {
    const Contacts = this.state.numbers.map((item, index) => (
      <Button
        ...
        key={index}
        onPress={() =>
          this.removingContactHandler(this.state.email, item.name, item.number)
        }
      >
        <Text style={styles.buttonText}>
          {item.name} : {item.number}
        </Text>
        <Icon name="ios-trash" color="black" />
      </Button>
    ));
